...
\author{James~R.~Wilburn% <-this % stops a space
        }
...
\begin{abstract}
%\boldmath
This project uses assertional reasoning to evaluate the properties of the PeerCube\cite{anceaume2008peercube} algorithm and modifies the algorithm to improve performance.  This is accomplished by identifying core properties of the alorithm. Once identified, these properties are maintained in the implementation even after modification. The major modifications to functionality include improved caching of data and mirroring of hot data to local nodes to minimize lookup times. Simulation and constrained testing verify these improvements preserve correctness while enhancing functionality.
\end{abstract}
...
\begin{IEEEkeywords}
PeerCube\cite{anceaume2008peercube}, hypercube, distributed hash table, assertional reasoning
\end{IEEEkeywords}
			...
			\begin{lrbox}{\mybox}
				\begin{minipage}{\hsize}
					...
					\eIf{$C.label$ = $p.cluster.label$}{
						...
						$p.RHC[key.str] \gets -1$\;
						$data \gets  p.RHD[key.str]$\;
						\Send{($'lookupreturn'$, key,data, p) to  q}
				  	}{
						\ForEach{$\varphi \in C.V_c$}{
						 	\Send{($'lookup'$, key, p, p) to  $\varphi$}\;
		...
		\begin{algorithm}
			...
			\begin{lrbox}{\mybox}
				\begin{minipage}{\hsize}
					...
					\Indentp{1.2em}
					\If{$p.RHC_{key.str} \neq -1$}{
						$p.RHD[key.str] \gets data$\;
						$p.RHC[key.str] \gets$ max($p.RHC[key.str] - 1, 0$)\;
					}
					% End your algorithm here
				\end{minipage}%
			\end{lrbox}
...
\begin{figure}
	...
	\begin{axis}[
	title={Mean requests per second (Higher is better)}, 
	    xlabel={},
	    enlarge x limits=0.20,
	    ylabel={Requests per second},
	    ybar,
	    legend style={at={(0.28,0.98)}},
	...
	\addplot[style={ppurple,fill=ppurple}] table [x expr=\coordindex,y=0mrps,col sep=comma] {data/caching.csv};
	\addplot[style={bblue,fill=bblue}] table [x expr=\coordindex,y=5mrps,col sep=comma] {data/caching.csv};
	\addplot[style={ggreen,fill=ggreen}] table [x expr=\coordindex,y=10mrps,col sep=comma] {data/caching.csv};
	\addplot[style={rred,fill=rred}] table [x expr=\coordindex,y=20mrps,col sep=comma] {data/caching.csv};
