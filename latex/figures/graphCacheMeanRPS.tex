...
\begin{document}
	\resizebox{\columnwidth}{!}{
		...
		\begin{tikzpicture}[scale=1]
			\begin{axis}[
			title={\caption{Mean Requests per Second\\(Replication)}\label{graph:CacheMeanRPS}}, 
			...
			\addplot[style={ppurple,fill=ppurple}] table [x expr=\coordindex,y=m0RPS,col sep=comma] {\cacheAVG};
			\addplot[style={bblue,fill=bblue}] table [x expr=\coordindex,y=m5RPS,col sep=comma] {\cacheAVG};
			\addplot[style={ggreen,fill=ggreen}] table [x expr=\coordindex,y=m10RPS,col sep=comma] {\cacheAVG};
			\addplot[style={rred,fill=rred}] table [x expr=\coordindex,y=m20RPS,col sep=comma] {\cacheAVG};
			\foreach \i in {0,1,...,3}{ % category loop
				\pgfplotstablegetelem{\i}{m0RPS}\of\cacheAVG
				\pgfmathsetmacro{\ya}{\pgfplotsretval}
				\pgfplotstablegetelem{\i}{m5RPS}\of\cacheAVG
				\pgfmathsetmacro{\yb}{\pgfplotsretval}
				\pgfplotstablegetelem{\i}{m10RPS}\of\cacheAVG
				\pgfmathsetmacro{\yc}{\pgfplotsretval}
				\pgfplotstablegetelem{\i}{m20RPS}\of\cacheAVG
