...
\begin{document}
	\resizebox{\columnwidth}{!}{
		...
		\begin{tikzpicture}[scale=1]
			\begin{axis}[
			title={\caption{Mean Requests per Second\\(Unmodified)}\label{graph:DefaultMeanRPS}}, 
			...
			\addplot[style={bblue,fill=bblue}] table [x expr=\coordindex,y=m5RPS,col sep=comma] {\defaultAVG};
			\addplot[style={ggreen,fill=ggreen}] table [x expr=\coordindex,y=m10RPS,col sep=comma] {\defaultAVG};
			\addplot[style={rred,fill=rred}] table [x expr=\coordindex,y=m20RPS,col sep=comma] {\defaultAVG};
			\foreach \i in {0,1,...,3}{ % category loop
				\pgfplotstablegetelem{\i}{m0RPS}\of\defaultAVG
				\pgfmathsetmacro{\ya}{\pgfplotsretval}
				\pgfplotstablegetelem{\i}{m5RPS}\of\defaultAVG
				\pgfmathsetmacro{\yb}{\pgfplotsretval}
				\pgfplotstablegetelem{\i}{m10RPS}\of\defaultAVG
				\pgfmathsetmacro{\yc}{\pgfplotsretval}
				\pgfplotstablegetelem{\i}{m20RPS}\of\defaultAVG
